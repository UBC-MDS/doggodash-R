...
# Col-2: Ranking plot
ranking_plot_fun <- function(top_5_raw_df){
  # Reading datasets
  ...
  top_5_rank_df <- rename_with(top_5_rank_df, ~ gsub(" Rank", "", .x, fixed=TRUE))
  
  top_5_rank_df <- top_5_rank_df %>%
...
ranking_plot <- 
  list(
    htmlP(
      'Ranking trend of Top 5 recommended breeds',
      style = list( 
        color= 'Indigo', 
        fontSize= 18,
        textAlign= 'center',
...
app$callback(
  output(id='ranking_plot', property = 'figure'),
  list(input(id='traits-widget', property='value')),
  function(traits){
    traits_df <- tibble::rowid_to_column(traits_raw_df, 'BreedID')
    breed_rank_df <- tibble::rowid_to_column(breed_rank_raw_df, "BreedID")
